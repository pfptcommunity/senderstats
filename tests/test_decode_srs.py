...
from senderstats.common.address_tools import convert_srs
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # srs appears only in domain -> ignore
        ("user@srs.example.com", "user@srs.example.com"),

        # srs in local but not at start or after '+'
        ("xxsrs0=AAA=BBB=orig.com=alice@example.net", "xxsrs0=AAA=BBB=orig.com=alice@example.net"),

        # srs at start but missing '=' pieces -> ignore
        ...
        # valid-ish (based on your current logic: take tokens[2] and tokens[3])
        ("srs0=AAA=BBB=orig.com=alice@example.net", "alice@orig.com"),
        ("base+srs0=AAA=BBB=orig.com=alice@example.net", "alice@orig.com"),

        # empty orig_local or orig_domain -> ignore
        ("srs0=AAA=BBB==alice@example.net", "srs0=AAA=BBB==alice@example.net"),
        ("srs0=AAA=BBB=orig.com=@example.net", "srs0=AAA=BBB=orig.com=@example.net"),
    ],
)
def test_convert_srs_unit_cases(inp: str, expected: str):
    assert convert_srs(inp) == expected
